...
- **MCP server** (`onepatch`, bundled with this plugin): tools appear as
  `mcp__onepatch__*`. First use triggers a browser sign-in; if calls fail with
  auth errors, tell the user to run `/mcp` and authenticate the `onepatch` server.
